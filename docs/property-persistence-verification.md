...
| Actually initialized model  | `gpt-5.6-luna` (verified from the live Codex process command line)      |
| Actually initialized effort | `max` (verified from `model_reasoning_effort=max` in that command line) |
| Fallback                    | None selected                                                           |

The initial environment-variable check exposed no model/effort fields; the later
process-level runtime audit supplied the command-line evidence recorded above.
...
collected. The existing compose database setup and volume were preserved.
...
All commands ran from this isolated worktree. Corepack reported pnpm `10.12.1`.
...
published on local port `5432`). No production database, deployment, commit, or push was
performed. The lockfile was regenerated/verified through the pinned `corepack.cmd pnpm`
path and is limited to dependency additions rather than whole-lockfile formatter churn.
...
of scope. The ordinary environment metadata did not expose initialization details, but the
live process command line verified the requested model and effort.
