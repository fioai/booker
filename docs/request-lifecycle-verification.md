...
## Scope and model metadata

This focused repair completes request-to-book lifecycle behavior for card `t_0f494ee8`:
bounded state transitions, immutable quote snapshots, tenant-scoped idempotent submission,
transactional internal holds, pending-only public submission, owner
approval/rejection/recheck operations, a PostgreSQL transactional outbox, and the
privacy-minimized public V1 boundary.

Requested model: `gpt-5.6-luna/max`.

Actual model initialization: unverified; this execution surface exposed no model metadata.

No commit, push, deploy, production service, production secret, production database, email,
or network delivery call was used. Outbox tests use only an in-process deterministic delivery
port.
