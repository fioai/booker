...
| Secrets and backups | Repository/image/log/history/archive exposure                                              | The local scanner checks tracked working-tree files, non-ignored untracked files, and staged index snapshots, and it redacts values. Ignored `.env`, dumps, archives, and backup directories require separate deliberate inspection and remain excluded from Git and Docker. Repository-host and Git-history scanning are separate controls. Deployments must encrypt, restrict, rotate, retain, and restore-test backups and secrets. |
...
- the local secret scan of tracked working-tree files, non-ignored untracked files, and staged
  ...
  suspected leak, repository-host and Git-history secret scanning; revoke or rotate every exposed
  credential and purge or rewrite history where required;
