...
// Exact historical fixtures only; see docs/security/secret-scanning.md for the review.
// A changed blob, path, line, or finding type must still fail the scan.
const HISTORICAL_PLACEHOLDERS = new Map([
  [
    '1659831f04c0d23b8af55a859f838613ce5644d2',
    { file: 'tests/hardening/hardening-contract.test.ts', line: 41 },
  ],
  [
    '76a535cfa1d6fdef3b094f499d821400ae2ac4e2',
    { file: 'tests/hardening/hardening-contract.test.ts', line: 41 },
  ],
  [
    '5d54ee034b91540a6415cf2dc6ad0abaac8c97e0',
    { file: 'tests/hardening/hardening-contract.test.ts', line: 41 },
  ],
  ['fa2933914b9a5ae4a906f648295b0dcc15ebc24a', { file: 'docker-compose.yml', line: 27 }],
]);
...
function scanBlob(aggregate, blob, contents) {
  ...
  const result = scanSecretCandidate(candidate);
  const exception = HISTORICAL_PLACEHOLDERS.get(blob.objectId);
  const reviewed = result.findings.filter(
    (finding) =>
      exception?.file === finding.file &&
      exception.line === finding.line &&
      finding.name === 'database-url-credential',
  );
  collectResult(aggregate, {
    ...result,
    findings: result.findings.filter((finding) => !reviewed.includes(finding)),
    totalFindings: result.totalFindings - reviewed.length,
    placeholders: [...result.placeholders, ...reviewed],
    totalPlaceholders: result.totalPlaceholders + reviewed.length,
  });
}
