...
export interface PostgresAdminCredentialStoreV1 extends AdminCredentialStoreV1 {
  create(input: AdminCredentialRecordV1): Promise<AdminCredentialRecordV1>;
  revokeMembership(organizationId: string, ownerId: string): Promise<boolean>;
}

export interface PostgresAdminSessionStoreV1 extends AdminSessionStoreV1 {
  create(user: AdminSessionUserV1): Promise<AdminSessionTicketV1>;
  get(token: string): Promise<AdminSessionV1 | null>;
...
function sessionFromRow(row: AdminSessionRow): AdminSessionV1 {
  const user = validateAdminSessionUserV1({
    id: row.owner_id as string,
    organizationId: row.organization_id as string,
    email: row.email as string,
    role: row.role as AdminSessionUserV1['role'],
  });
...
export function createPostgresAdminCredentialStoreV1(
  database: PostgresDatabasePort,
): PostgresAdminCredentialStoreV1 {
  return createOwnerCredentialRepository(database) as unknown as PostgresAdminCredentialStoreV1;
}

export function createPostgresAdminSessionStoreV1(
  database: PostgresDatabasePort,
  options: AdminSessionStoreOptionsV1 = {},
): PostgresAdminSessionStoreV1 {
  ...
  return {
    async create(user: AdminSessionUserV1): Promise<AdminSessionTicketV1> {
    ...
    async get(value: string): Promise<AdminSessionV1 | null> {
