...
export interface AdminHttpServerOptions extends AdminHttpApiOptions {
  readonly dependencies: AdminHttpApiDependencies;
}

export interface AdminHttpServerAddress {
...
export interface AdminHttpServer {
  ...
  listen(port?: number, host?: string): Promise<AdminHttpServerAddress>;
  close(): Promise<void>;
}
...
function writeResponse(response: ServerResponse, result: AdminHttpResponse): void {
...
export function createAdminHttpServer(options: AdminHttpServerOptions): AdminHttpServer;
export function createAdminHttpServer(
  dependencies: AdminHttpApiDependencies,
  options?: AdminHttpApiOptions,
): AdminHttpServer;
export function createAdminHttpServer(
  dependenciesOrOptions: AdminHttpServerOptions | AdminHttpApiDependencies,
  options: AdminHttpApiOptions = {},
): AdminHttpServer {
  const resolvedOptions: AdminHttpServerOptions =
    'dependencies' in dependenciesOrOptions
      ? dependenciesOrOptions
      : { ...options, dependencies: dependenciesOrOptions };
  const api = createAdminHttpApi(resolvedOptions.dependencies, resolvedOptions);
  ...
  return {
    server,
    listen(port = 0, host = '127.0.0.1'): Promise<AdminHttpServerAddress> {
      return new Promise((resolve, reject) => {
