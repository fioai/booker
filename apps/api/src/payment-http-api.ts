...
export interface PaymentHttpRequestV1 {
...
export interface PaymentHttpResponseV1 {
  readonly status: number;
  readonly body: unknown;
}

export interface PaymentHttpApiOptionsV1 {
  /** Tenant scope is supplied by the trusted composition boundary, never the browser. */
  readonly scope: PaymentOrganizationScopeV1;
}

export interface PaymentHttpApiV1 {
  handle(request: PaymentHttpRequestV1): Promise<PaymentHttpResponseV1>;
}

class PaymentHttpErrorV1 extends Error {
  readonly status: number;
  readonly code:
    | 'invalid_input'
    | 'payment_unavailable'
  ...
  constructor(status: number, code: PaymentHttpErrorV1['code'], message: string) {
    super(message);
    this.name = 'PaymentHttpErrorV1';
    this.status = status;
    this.code = code;
  }
}

function errorResponse(error: unknown): PaymentHttpResponseV1 {
  if (error instanceof PaymentHttpErrorV1) {
    return {
      status: error.status,
...
function checkoutResponse(session: CheckoutSession): PaymentHttpResponseV1 {
  return {
    status: 201,
    body: {
...
export function createPaymentHttpApiV1(
  service: PaymentCheckoutServiceV1,
  options: PaymentHttpApiOptionsV1,
): PaymentHttpApiV1 {
  ...
  return {
    async handle(request): Promise<PaymentHttpResponseV1> {
      const parsedRoute = route(request.path);
      if (parsedRoute === undefined) {
        return errorResponse(
          new PaymentHttpErrorV1(404, 'route_not_found', 'Payment route was not found.'),
        );
      }
      try {
        if (parsedRoute.kind === 'checkout') {
          if (request.method !== 'POST') {
            throw new PaymentHttpErrorV1(
              405,
              'method_not_allowed',
