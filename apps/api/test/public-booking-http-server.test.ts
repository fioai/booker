...
import type { PublicRequestToBookInputV1 } from '@lotus-booking/sdk-typescript';
...
function requestRecordForHttp(): BookingRequestRecordV1 {
  return {
    id: 'request-http-001',
...
function dependencies() {
  ...
  const bookingRequests: PublicBookingRequestRepositoryV1 = {
    submit: vi.fn(
      async (_scope, requestedPropertyId, input): Promise<BookingRequestRecordV1> => ({
        ...input,
        organizationId: scope.organizationId,
...
describe('public booking v1 real HTTP server', () => {
  ...
  it('serves OpenAPI and every v1 route through a bound ephemeral server', async () => {
    server = createPublicBookingHttpServerV1(dependencies(), { scope });
  ...
  it('passes the idempotency key and pending-only mode to the request boundary', async () => {
    ...
    server = createPublicBookingHttpServerV1(deps, { scope });
    const address = await server.listen(0);
    baseUrl = `http://127.0.0.1:${address.port}`;
