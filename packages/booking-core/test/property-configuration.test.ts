...
import {
  ...
  PROPERTY_CONFIGURATION_LIMITS_V1,
  type PropertyConfigurationInputV1,
  type PropertyConfigurationV1,
} from '@lotus-booking/booking-core';
import { sampleBungalowFixture, syntheticHarbourLoftFixture } from './property-fixtures.js';

function withChanges(changes: Partial<PropertyConfigurationInputV1>): PropertyConfigurationInputV1 {
...
describe('PropertyConfigurationV1', () => {
  it.each([
    ...
    const result = createPropertyConfigurationV1(fixture);
    ...
    expect(isPropertyConfigurationV1(result.value)).toBe(true);
  ...
  it('rejects structural objects as validated configurations at runtime', () => {
    const configuration = validConfiguration();
    const forged = { ...configuration } as unknown as PropertyConfigurationV1;
  ...
  it('rejects empty and oversized identifiers', () => {
    ...
    expectErrors(
      withChanges({
        id: 'x'.repeat(PROPERTY_CONFIGURATION_LIMITS_V1.propertyIdMaxLength + 1),
      }),
      [['id', 'string_too_long']],
    );
  ...
  it('rejects empty and oversized names', () => {
    ...
    expectErrors(
      withChanges({
        name: 'x'.repeat(PROPERTY_CONFIGURATION_LIMITS_V1.nameMaxLength + 1),
      }),
      [['name', 'string_too_long']],
    );
    expectErrors(
      withChanges({
        summary: 'x'.repeat(PROPERTY_CONFIGURATION_LIMITS_V1.summaryMaxLength + 1),
      }),
  ...
  it('rejects invalid country, currency, and timezone values exactly', () => {
    ...
    expectErrors(
      withChanges({
        timezone: 'x'.repeat(PROPERTY_CONFIGURATION_LIMITS_V1.timezoneMaxLength + 1),
      }),
  ...
  it('rejects unsupported and oversized property types', () => {
    ...
    expectErrors(
      withChanges({
        propertyType: 'x'.repeat(PROPERTY_CONFIGURATION_LIMITS_V1.propertyTypeMaxLength + 1),
      }),
  ...
  it('rejects negative, non-integer, and excessive counts', () => {
    ...
    expectErrors(
      withChanges({
        bedroomCount: PROPERTY_CONFIGURATION_LIMITS_V1.maxBedroomCount + 1,
      }),
    ...
    expectErrors(
      withChanges({
        bathroomCount: PROPERTY_CONFIGURATION_LIMITS_V1.maxBathroomCount + 1,
      }),
    ...
    expectErrors(
      withChanges({
        maximumGuests: PROPERTY_CONFIGURATION_LIMITS_V1.maxGuests + 1,
      }),
    ...
    expectErrors(
      withChanges({
        bedConfiguration: [
          {
            type: 'queen',
            quantity: PROPERTY_CONFIGURATION_LIMITS_V1.maxBedQuantity + 1,
          },
        ],
      }),
  ...
  it('rejects duplicate and oversized amenities', () => {
    ...
    expectErrors(
      withChanges({
        amenities: Array.from(
          { length: PROPERTY_CONFIGURATION_LIMITS_V1.maxAmenities + 1 },
    ...
    expectErrors(
      withChanges({
        amenities: ['x'.repeat(PROPERTY_CONFIGURATION_LIMITS_V1.amenityMaxLength + 1)],
      }),
  ...
  it('does not traverse entries beyond an oversized array bound', () => {
    ...
    oversizedBeds.length = PROPERTY_CONFIGURATION_LIMITS_V1.maxBedConfigurations + 1;
    Object.defineProperty(oversizedBeds, PROPERTY_CONFIGURATION_LIMITS_V1.maxBedConfigurations, {
      get() {
    ...
    oversizedAmenities.length = PROPERTY_CONFIGURATION_LIMITS_V1.maxAmenities + 1;
    Object.defineProperty(oversizedAmenities, PROPERTY_CONFIGURATION_LIMITS_V1.maxAmenities, {
      get() {
  ...
  it('accepts null-prototype own-property records', () => {
    ...
    const input = inputRecord as unknown as PropertyConfigurationInputV1;

    expect(createPropertyConfigurationV1(input).ok).toBe(true);
  });
  ...
  it('rejects oversized public and private notes', () => {
    expectErrors(
      withChanges({
        hostNotes: 'x'.repeat(PROPERTY_CONFIGURATION_LIMITS_V1.hostNotesMaxLength + 1),
      }),
      [['hostNotes', 'string_too_long']],
    );
    expectErrors(
      withChanges({
        operationalNotes: 'x'.repeat(
          PROPERTY_CONFIGURATION_LIMITS_V1.operationalNotesMaxLength + 1,
        ),
      }),
