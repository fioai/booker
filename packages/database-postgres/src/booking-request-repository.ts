...
import {
  ...
  type BookingRequestActionV1,
  type BookingRequestStatusV1,
  type QuoteBreakdownV1,
  type QuoteSnapshotValidationErrorV1,
...
export interface BookingRequestOrganizationScopeV1 {
  readonly organizationId: string;
}

export interface BookingRequestCreateInputV1 {
  readonly id: string;
...
export interface BookingRequestSubmitOptionsV1 {
...
export interface BookingRequestRepositoryOptionsV1 {
  readonly clock?: () => Date;
  readonly holdDurationMs?: number;
}

export interface BookingRequestRecordV1 extends BookingRequestCreateInputV1 {
  readonly organizationId: string;
  readonly propertyId: string;
  readonly status: BookingRequestStatusV1;
...
export interface BookingRequestRecheckResultV1 {
  readonly request: BookingRequestRecordV1;
  readonly available: boolean;
}

export interface BookingRequestRepositoryV1 {
  create(
    scope: BookingRequestOrganizationScopeV1,
    propertyId: string,
    input: BookingRequestCreateInputV1,
  ): Promise<BookingRequestRecordV1>;
  submit(
    scope: BookingRequestOrganizationScopeV1,
    propertyId: string,
    input: BookingRequestCreateInputV1,
    options: BookingRequestSubmitOptionsV1 | string,
  ): Promise<BookingRequestRecordV1>;
  find(
    scope: BookingRequestOrganizationScopeV1,
    propertyId: string,
    requestId: string,
  ): Promise<BookingRequestRecordV1 | null>;
  list(
    scope: BookingRequestOrganizationScopeV1,
    propertyId: string,
  ...
  recheckAvailability(
    ...
    propertyId: string,
    requestId: string,
  ): Promise<BookingRequestRecheckResultV1>;
  expire(
    scope: BookingRequestOrganizationScopeV1,
    propertyId: string,
    requestId: string,
...
function validateScope(scope: BookingRequestOrganizationScopeV1): string {
...
function validateInput(input: BookingRequestCreateInputV1): BookingRequestCreateInputV1 & {
  readonly quote: QuoteBreakdownV1;
} {
  ...
  const snapshot = createQuoteSnapshotV1(input.quote);
  if (!snapshot.ok) {
    throw new PersistenceError(
      'booking_request_validation',
      'server quote snapshot failed validation.',
      snapshot.errors as readonly QuoteSnapshotValidationErrorV1[],
    );
  }
  if (
...
function fingerprint(propertyId: string, input: BookingRequestCreateInputV1): string {
...
function mapRow(row: BookingRequestRow): BookingRequestRecordV1 {
  if (
  ...
  const snapshot = createQuoteSnapshotV1(row.quote_json);
  if (!snapshot.ok) {
    throw new PersistenceError(
      'database_corruption',
...
function asOptions(options: BookingRequestSubmitOptionsV1 | string): BookingRequestSubmitOptionsV1 {
...
async function insertOutbox(
  ...
  propertyId: string,
  requestId: string,
  eventType: BookingOutboxEventTypeV1,
  status: BookingRequestStatusV1,
  quote: QuoteBreakdownV1,
  at: Date,
): Promise<void> {
  await transaction.query(
    `
...
export class PostgresBookingRequestRepository implements BookingRequestRepositoryV1 {
  ...
  constructor(
    ...
    options: BookingRequestRepositoryOptionsV1 = {},
  ) {
  ...
  private isExpired(record: BookingRequestRecordV1, at: Date): boolean {
    return (
  ...
  private async transition(
    ...
    try {
      return await this.database.withTransaction(async (transaction) => {
        ...
        const next = transitionBookingRequestV1(record.status, action);
        if (!next.ok) {
          throw new PersistenceError(
            'invalid_booking_request_transition',
            `cannot ${action} a ${record.status} booking request.`,
  ...
  private async expireLoaded(
    ...
    const next = transitionBookingRequestV1(record.status, 'expire');
    if (!next.ok) {
      throw new PersistenceError(
        'invalid_booking_request_transition',
