...
export type {
  PaymentCheckoutPreparationV1,
  PaymentCheckoutRecordV1,
  PaymentOrganizationScopeV1,
  PaymentProviderRegistrationV1,
  PaymentWebhookProcessingResultV1,
  PaymentWebhookProcessingStatusV1,
} from '@lotus-booking/payments';

export interface PaymentCheckoutRepositoryOptionsV1 {
  readonly clock?: () => Date;
  readonly checkoutDurationMs?: number;
  readonly maxCheckoutAttempts?: number;
}

export interface PaymentCheckoutRepositoryV1 extends PaymentCheckoutStoreV1 {
  findCheckout(
    scope: PaymentOrganizationScopeV1,
    propertyId: string,
    checkoutId: string,
  ): Promise<PaymentCheckoutRecordV1 | null>;
}
...
function validateProvider(value: PaymentProviderRegistrationV1): PaymentProviderRegistrationV1 {
  if (
    typeof value !== 'object' ||
    value === null ||
...
function mapState(value: unknown): PaymentStateV1 {
  if (
    value !== 'created' &&
    value !== 'open' &&
    value !== 'paid' &&
...
function mapCheckout(row: PaymentCheckoutRow): PaymentCheckoutRecordV1 {
  if (
...
function quoteRevision(quote: QuoteBreakdownV1): string {
  return createHash('sha256').update(JSON.stringify(quote)).digest('hex');
}

function webhookEventHash(event: PaymentWebhookEventV1): string {
...
function validateWebhookEvent(event: PaymentWebhookEventV1): PaymentWebhookEventV1 {
  if (
    typeof event !== 'object' ||
    event === null ||
...
export class PostgresPaymentCheckoutRepository implements PaymentCheckoutRepositoryV1 {
  ...
  constructor(
    ...
    options: PaymentCheckoutRepositoryOptionsV1 = {},
  ) {
  ...
  async prepareCheckout(
    ...
    propertyId: string,
    requestId: string,
    providerInput: PaymentProviderRegistrationV1,
    ...
    try {
      return await this.database.withTransaction(async (transaction) => {
        ...
        const quote = createQuoteSnapshotV1(requestRow.quote_json);
        if (!quote.ok) {
          throw new PersistenceError(
            'database_corruption',
        ...
        if (existing !== undefined) {
          ...
          const existingRequestResult = createPaymentCheckoutRequestV1({
          ...
          return {
            ...
            state: mapped.state as Extract<PaymentStateV1, 'created' | 'open'>,
        ...
        const checkoutRequestResult = createPaymentCheckoutRequestV1({
          organizationId,
          propertyId: property,
          requestId: request,
  ...
  async processWebhookEvent(
    ...
    return this.database.withTransaction(async (transaction) => {
      ...
      const transition = paymentStateTransitionV1(payment.state, event.eventType);
      if (
        !transition.ok ||
        payment.state === 'paid' ||
  ...
  private paymentMismatchCode(
    payment: PaymentCheckoutRecordV1,
    event: PaymentWebhookEventV1,
  ): string | null {
  ...
  private async markEvent(
    transaction: PostgresTransactionPort,
    event: PaymentWebhookEventV1,
    status: PaymentWebhookProcessingStatusV1,
    code: string | null,
    at: Date,
  ): Promise<void> {
    await transaction.query(
