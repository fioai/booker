...
export class PostgresRateRepository implements RateRepository {
  ...
  async getRatePlan(scope: RateOrganizationScope, propertyId: string): Promise<RatePlanV1 | null> {
    ...
    await this.database.withTransaction((transaction) =>
      requirePropertyQuery(transaction, this.propertiesTable, organizationId, id),
    );

    const planResult = await this.database.query<RatePlanRow>(
      `
    ...
    const overrides = await this.database.query<SeasonalRateRow>(
      `
...
export function createRateRepository(database: PostgresDatabasePort): RateRepository {
  return new PostgresRateRepository(database);
}

export const createPostgresRateRepository = createRateRepository;
